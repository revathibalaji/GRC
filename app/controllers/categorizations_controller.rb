...
#  end
  ...
  private
    ...
    def list_form_context
      super.merge \
        :object => params[:object_type].constantize.find_by_id(params[:object_id]),
        :scope_id => params[:scope_id]
    end

    def update_object(relation, object_params)
      relation.category = Category.find(object_params[:category_id])
      related_object = object_params[:categorizable_type].constantize.find_by_id(object_params[:categorizable_id])
